...
There are two classes of users that have access to the API - "account" class users and "contact" class users.  A contact class user is unable to access many API resources (see individual resource documentaiton for details) and must include a x-zingle-contact-id header in every request to the API with the exception of the contact_services resource and the contact resource's create method.  
